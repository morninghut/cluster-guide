...
* 使用VSCode连接集群
    ...
    这部分其实也没啥好说的，就是在本地安装VSCode，安装Remote-SSH插件，然后连接集群。如果需要详细的教程，请自行搜索。Pycharm连接集群也很类似。

* 关于vim
    如果有其他人看到这篇文档，也许很想打我：“你为什么只是echo？为什么只是cat？why not use vim？”
    ~~你说得对，但是vim是...~~
    原因是本教程的受众是没有Linux基础的新手。vim本身也足够复杂，没有Linux基础的话一开始更会觉得难以适应（搜索如何退出vim.jpg）。为了降低学习成本，我使用了echo和cat。如果你已经有了一定的Linux基础，我非常推荐你学习vim，它是一个非常强大的编辑器。入门推荐使用vimtutor，完成这个程序的任务后基本就算入了vim的门。
